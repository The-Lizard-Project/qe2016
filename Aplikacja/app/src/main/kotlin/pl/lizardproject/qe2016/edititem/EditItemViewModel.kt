...
class EditItemViewModel(val itemId: String?, private val activity: Activity, private val databaseFacade:
DatabaseFacade) {
    ...
    fun saveItemCommand(view: View) {
        if (!TextUtils.isEmpty(newItemName.get())) {
            databaseFacade.saveItem(Item(itemId ?: "", newItemName.get(), Category.values()[newItemCategoryPosition.get()
                    ], Priority.values()[newItemPriorityPosition.get()]))
            activity.finish()
        } else {
